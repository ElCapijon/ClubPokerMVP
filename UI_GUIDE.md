...
|---|---|---|---|
| Your seat cards (`.hero-seat-cards`, behind the avatar) | **44×64** (lg size prop) | **30×52** | *(not on the felt)* |
...
**Your hole cards render on the felt too** — anchored directly to the **avatar ring** (rendered *inside* the seat column, `top: calc(50% - 16px)` of the ring) and placed **behind** the profile image: the cards sit at `z-0` inside the ring (which is `relative z-10`), so the avatar paints over the middle of the pair while the tops peek out **above** it. The 16px nudge is what makes it work — it puts the **rank corners clearly above the avatar ring**, so the visible part of the hand is instantly readable, with the rest tucked "behind" the avatar (the classic cards-held-up-to-your-chest look). Anchoring to the ring (not the seat column) means the cards always track the avatar exactly, no matter how the D button, name, stack, or badges change the column's height.

Because the cards now occupy the space above the avatar, the **dealer button on your own seat no longer sits in the column stack** — it floats **beside the avatar** (absolutely positioned at the avatar's vertical center, `-left-10 sm:-left-14` on center/right seats, `-right-10 sm:-right-14` on left seats, so side seats face the table center). That keeps it clear of the card tops. Other seats keep the D above the avatar, unchanged.
...
- **Why do your hole cards live on the felt at your seat?** The old bottom-bar row (64×90 on desktop, 44×64 on portrait) cost a full row of vertical budget on every screen. Moving the cards to the felt reclaimed that row for the table (roughly 100px on desktop). **Why behind the avatar rather than floating on the felt?** Positioning them anywhere on the seat→pot line read as "too high up the table", and there is physically no room to lower them toward the seat (the bet chip and the seat column block it). Anchoring them to the **avatar ring** reads as truly at your seat. **Why the 16px up-nudge?** The first behind-the-avatar attempt centered the pair on the *seat column* — which sits below the avatar (the D button, name, stack and badges push the column's center down) — so the visible peek hung low over the name and the avatar covered almost everything, and the dealer button sat exactly where the cards' tops should have been. Nudging the cards **above** the avatar (rank corners clear the ring, with a touch of lean toward the table center) plus moving **your** dealer button to float **beside** the avatar fixed all three. The pair keeps a fan + gold glow + turn-scale so it still reads clearly at a glance. On landscape phones they stay in the compact bottom bar because the flat 2:1 felt is too short to host them.
